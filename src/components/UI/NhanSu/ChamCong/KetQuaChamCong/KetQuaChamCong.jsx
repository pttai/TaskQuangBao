...
const KetQuaChamCong = (props) => {
  ...
  useEffect(() => {
    ...
    axios({
      method: 'GET',
      url: 'https://quanlyquangbao.herokuapp.com/api/chamcong/danhsachchamcong',
    }).then((res) => {
      const { data } = res.data;
      const datas = [];
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'sogiolam',
      key: 'sogiolam',
      sorter: (a, b) => a.sogiolam - b.sogiolam,
    },
    {
      title: 'Tên Ca',
      dataIndex: ['idca', 'tenca'],
      key: ['idca', 'tenca'],
    },
    // {
    //   title: 'Tùy Chỉnh',
  ...
  return (
    <div>
      <Breadcrumb style={{ margin: '16px 0', fontSize: 20 }}>
        ...
        <Breadcrumb.Item>Kết Quả Chấm Công</Breadcrumb.Item>
      </Breadcrumb>
      {/* <Button
        tyle='primary'
        style={{
